...
async function getProducer(): Promise<Producer> {
    ...
    await _producer.connect(); return _producer;
}
export function publishTripEvent(topic: string, payload: Record<string, unknown>): void {
    getProducer().then(p => p.send({
        topic, compression: CompressionTypes.GZIP,
        messages: [{
            key: String(payload['tripId'] || 'unknown'),
